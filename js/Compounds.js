AFRAME.registerComponent("atoms", {
  ...
  createAtoms: async function (element) {
    ...
    for (var num=1;num<=numOfElectron;num++) {
      ...
      orbit.setAttribute( 'material',{color:"purple",opacity:0.5} );
      orbit.setAttribute('position', {x:0,y:1,z:0})
      orbit.setAttribute('rotation',{x:0,y:orbitangle,z:0});
      orbitangle+=45
      atom.appendChild(orbit)
      var electroneangle=30
      var electronegroup=document.createElement('a-entity')
      electronegroup.setAttribute('id',`electrone-group-${elementName}`)
      electronegroup.setAttribute('rotation',{x:0,y:0,z:electroneangle})
      electroneangle+=65
      electronegroup.setAttribute( 'animation',{property:"rotation",to:`0 0 -360`,loop:"true",dur:3500,easing:"linear"});
